...
\begin{itemize}
  ...
  \item Reversal: reverse all transitions and swap start/accept to get an NFA (possibly with multiple start states or $\varepsilon$-transitions), then determinize
\end{itemize}
\end{theorem}
...
\begin{example}[Subset construction]
Convert the following NFA for ``strings over $\{0,1\}$ that contain the substring $01$'' into a DFA.

\emph{NFA.} States $q_0$ (start), $q_1$ (just saw a 0), $q_2$ (accept). Transitions:
\begin{itemize}
  \item $q_0$ on 0 $\to \{q_0, q_1\}$, on 1 $\to \{q_0\}$
  \item $q_1$ on 1 $\to \{q_2\}$, on 0 $\to \emptyset$
  \item $q_2$ on 0 or 1 $\to \{q_2\}$ (once seen, stay accepting)
\end{itemize}

\emph{DFA states (subsets).} Start with $\{q_0\}$. The reachable subsets are:
\[
\{q_0\}, \{q_0,q_1\}, \{q_0,q_2\}, \{q_0,q_1,q_2\}.
\]
Accepting subsets are those containing $q_2$.
...
\end{tabular}
\end{center}
This DFA recognizes the same language.
\end{example}
