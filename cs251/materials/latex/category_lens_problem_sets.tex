...
\end{enumerate}
\textbf{Hint:} A map $1 \to A$ just chooses a single element of $A$. For the product, compute $\langle f,g \rangle(x) = (f(x), g(x))$ for each $x \in X$.
...
\end{enumerate}
\textbf{Hint:} For (b), take $B$ to be the image of $p$ and let $r$ be $p$ with codomain restricted to $B$; let $i$ be the inclusion $B \hookrightarrow A$.
...
\end{enumerate}
\textbf{Hint:} In divisibility, meet/join correspond to $\gcd$ and $\text{lcm}$. For monotonicity, show $S \subseteq T \Rightarrow |S| \leq |T|$.
...
\end{enumerate}
\textbf{Hint:} For (b), $\tilde{g}(x)$ and $\tilde{g}(y)$ are functions $A \to B$; write each as a table on inputs 0 and 1.
...
\end{enumerate}
\textbf{Hint:} The number of maps with exactly two 1s is the number of 2-element subsets of $A$.
...
\end{enumerate}
\textbf{Hint:} A graph homomorphism must send every edge to an edge. To find a non-example, map an edge in $G$ to a non-edge in $H$.
...
\end{enumerate}
\textbf{Hint:} In a path graph, there is exactly one directed path from 1 to 3. For $\pi_0(G)$, list which vertices are connected by any path.
...
\end{enumerate}
\textbf{Hint:} Use $\textit{leaf}(a)=1$ and $\textit{node}(x,y)=x+y$. The example tree has three leaves.
...
\end{enumerate}
\textbf{Hint:} Track the parity of the number of 1s. For $\delta^*$, step through the string one symbol at a time.
...
\end{enumerate}
\textbf{Hint:} A monoid homomorphism is determined by its values on generators. In the one-object category, morphisms are natural numbers and composition is addition.
