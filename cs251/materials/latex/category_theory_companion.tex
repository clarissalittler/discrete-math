...
\subsection{Natural Transformations (Brief)}

Functors translate between categories. A \textbf{natural transformation} translates between functors.

\begin{definition}[Natural transformation]
Given functors $F, G: \mathcal{C} \to \mathcal{D}$, a \textbf{natural transformation} $\eta: F \Rightarrow G$ assigns to each object $A$ a morphism $\eta_A: F(A) \to G(A)$ such that for every $f: A \to B$:
\[
G(f) \circ \eta_A = \eta_B \circ F(f).
\]
This is the \emph{naturality} condition.
\end{definition}

\begin{example}[Singleton map]
Let $F = \mathrm{Id}_{\Set}$ and $G = \Pow$. Define $\eta_A: A \to \Pow(A)$ by $\eta_A(a) = \{a\}$.
For any $f: A \to B$,
\[
\Pow(f)(\eta_A(a)) = \Pow(f)(\{a\}) = \{f(a)\} = \eta_B(f(a)),
\]
so $\Pow(f) \circ \eta_A = \eta_B \circ f$. Thus $\eta$ is a natural transformation $F \Rightarrow G$.
\end{example}
