...
A set $A$ is \textbf{countably infinite} if there is a bijection $A \leftrightarrow \N$. In other words, you can list its elements: $a_0, a_1, a_2, \ldots$ with nothing missing and no repeats.
...
\par\noindent\textbf{Convention:} Throughout these notes, $\N = \{0, 1, 2, \ldots\}$.
\[
0, 1, -1, 2, -2, 3, -3, \ldots
\]
...
\emph{Proof (sketch).} Consider pairs $(p, q)$ with $p \in \Z$ and $q \in \N^+$. We can list all such pairs by diagonals:
\[
|p| + q = 1, 2, 3, \ldots
\]
Each diagonal has finitely many pairs, so the whole set $\Z \times \N^+$ is countable.

Define $h: \Z \times \N^+ \to \Q$ by $h(p, q) = p/q$. This hits every rational (not uniquely, because $2/2 = 1/1$), so $\Q$ is the image of a countable set, hence countable.

If you want a bijection, restrict to \emph{reduced} pairs with $\gcd(p, q) = 1$ and $q > 0$. That subset is still countable, and now $h$ is one-to-one. \qed
\end{example}
...
\begin{keyresult}[Most functions are incomputable]
Here's a striking application of cardinality to computer science: \textbf{almost all} functions from $\N$ to $\N$ cannot be computed by any program.

The argument is beautifully simple, requiring only two facts about cardinality.

\textbf{Fact 1: There are uncountably many functions $\N \to \N$.}

Consider the set of all functions $f: \N \to \N$. By the general formula for function spaces, $|A \to B| = |B|^{|A|}$, so $|\N \to \N| = \aleph_0^{\aleph_0}$. We'll show this equals $|\Pow(\N)| = 2^{\aleph_0} = |\R|$.

For any subset $S \subseteq \N$, define its \emph{characteristic function} $\chi_S: \N \to \N$ by:
\[
\chi_S(n) = \begin{cases} 1 & \text{if } n \in S \\ 0 & \text{if } n \notin S \end{cases}
\]
This gives an injection from $\Pow(\N)$ into $(\N \to \N)$: different subsets have different characteristic functions. So $|\Pow(\N)| \leq |\N \to \N|$.

Conversely, we can encode any function $f: \N \to \N$ as a subset of $\N \times \N$ (its graph), and $|\N \times \N| = |\N|$, so $|\N \to \N| \leq |\Pow(\N \times \N)| = |\Pow(\N)|$.

Therefore $|\N \to \N| = \aleph_0^{\aleph_0} = |\Pow(\N)| = 2^{\aleph_0} = |\R|$, which is uncountable.

\textbf{Fact 2: There are only countably many programs.}
...
\begin{itemize}
  \item There are finitely many strings of length 0 (just the empty string).
  \item There are finitely many strings of length 1 (one for each alphabet symbol).
  \item There are finitely many strings of length 2, length 3, and so on.
\end{itemize}
...
\[
|\{\text{computable functions } \N \to \N\}| \leq |\{\text{programs}\}| = \aleph_0
\]

But we just showed $|\{\text{all functions } \N \to \N\}| = 2^{\aleph_0}$, which is strictly larger than $\aleph_0$.

Therefore: there are uncountably many functions, but only countably many programs. The computable functions are a \emph{measure-zero} subset of all functions. In a very precise sense, if you picked a random function $\N \to \N$, the probability that it's computable is zero.

This argument doesn't tell us \emph{which} functions are incomputable---for that, you need specific examples like the halting problem. But it tells us that incomputability is the norm, not the exception. The functions we can actually compute are infinitesimally rare among all the functions that mathematically exist.
\end{keyresult}
...
In $\Set$, a function has a section iff it's surjective (you can ``pick'' a preimage for each output). If $A$ is nonempty, then $f$ has a retraction iff it's injective; if $A = \emptyset$, a retraction exists only when $B = \emptyset$.
