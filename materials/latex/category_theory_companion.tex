...
$G(A) = \{\text{simple graphs with vertex set } A\}$. We have $|G(\{1,2,3\})| = 2^3 = 8$.
\end{example}
...
\begin{exercise}
Compute the Brzozowski derivatives $\partial_0(L)$ and $\partial_1(L)$ for $L = \{w : w \text{ has even number of 1s}\}$.
\end{exercise}
...
\textbf{3. Duality.} Many concepts come in pairs: products/coproducts, monos/epis, algebras/coalgebras, initial/final. Understanding one often gives the other ``for free.''
