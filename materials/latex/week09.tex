...
\begin{itemize}
  ...
  \item $\Sigma$ is a set of terminals (alphabet)
  \item $R$ is a set of production rules of the form $A \to \alpha$, where $A \in V$ and $\alpha \in (V \cup \Sigma)^*$
  \item $S \in V$ is the start symbol
\end{itemize}
The language $L(G)$ is the set of strings in $\Sigma^*$ derivable from $S$.
\end{definition}

\begin{definition}[Derivation and parse tree]
We write $S \Rightarrow^* w$ if $w$ can be derived from $S$ by repeatedly applying production rules.
A \textbf{parse tree} records the rule applications that produce $w$.
\end{definition}

\begin{example}
The grammar with rules $S \to aSb \mid \varepsilon$ generates $L = \{a^n b^n : n \geq 0\}$.
For example, $S \Rightarrow aSb \Rightarrow aaSbb \Rightarrow aabb$.
\end{example}

\begin{remark}
A grammar is \textbf{ambiguous} if some string has two different parse trees. Ambiguity complicates parsing and language reasoning.
\end{remark}

\subsection*{Pushdown automata}

\begin{definition}[Pushdown automaton (PDA)]
A \textbf{pushdown automaton} is like an NFA with a stack. Formally it is a 6-tuple
$(Q, \Sigma, \Gamma, \delta, q_0, F)$ where $\Gamma$ is the stack alphabet and
$\delta$ reads an input symbol (or $\varepsilon$) and the top of the stack, then updates the state and stack.
\end{definition}

\begin{theorem}[CFG-PDA equivalence]
A language is context-free iff it is recognized by a pushdown automaton.
\end{theorem}
...
\subsubsection*{Automata as Actions of the Free Monoid}
The set of all strings $\Sigma^*$ is the \textbf{free monoid} on $\Sigma$. A DFA induces a transition action:
\[
...
\begin{enumerate}
  ...
  \item Give a CFG for $L = \{0^n1^n : n \geq 0\}$ and derive the string $000111$.

  \item Show that the grammar with rules $S \to SS \mid a$ is ambiguous by finding two distinct parse trees for $aa$.

  \item Describe (in words) a PDA that recognizes $L = \{a^n b^n : n \geq 0\}$, and trace its stack contents on input $aabb$.
\end{enumerate}
