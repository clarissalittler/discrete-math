...
\subsubsection*{Isomorphisms, Sections, and Retractions}
A map $f: A \to B$ is an \textbf{isomorphism} if there exists $g: B \to A$ with $g \circ f = \id_A$ and $f \circ g = \id_B$. In $\Set$, isomorphisms are exactly bijections.

A \textbf{section} (right inverse) is a map $s: B \to A$ with $f \circ s = \id_B$. A \textbf{retraction} (left inverse) is a map $r: B \to A$ with $r \circ f = \id_A$:
\[
\begin{tikzcd}
...
\subsubsection*{Idempotents and Retracts}
A map $p: A \to A$ is \textbf{idempotent} if $p \circ p = p$. In $\Set$, idempotents correspond to ``projection onto a subset'': if $p = i \circ r$ with $r \circ i = \id_B$, then $p$ projects $A$ onto the retract $B$.
