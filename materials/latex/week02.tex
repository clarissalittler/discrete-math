...
A map $f: A \to B$ is an \textbf{isomorphism} if there exists $g: B \to A$ with $g \circ f = \id_A$ and $f \circ g = \id_B$. In $\Set$, isomorphisms are exactly the bijections.

But we can have one-sided inverses:
\begin{itemize}
  \item A \textbf{section} (right inverse) is $s: B \to A$ with $f \circ s = \id_B$. This forces $f$ to be surjective.
  \item A \textbf{retraction} (left inverse) is $r: B \to A$ with $r \circ f = \id_A$. This forces $f$ to be injective.
\end{itemize}
