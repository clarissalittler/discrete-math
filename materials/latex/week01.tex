...
In $\Set$, a singleton set $1 = \{*\}$ is special. Every element $a \in A$ corresponds to a unique map $1 \to A$ that sends $*$ to $a$. In categorical language, \emph{elements of $A$ are its points}, i.e., arrows $1 \to A$.

\subsubsection*{Terminal and Initial Objects}
The singleton set $1$ is a \textbf{terminal object}: for every set $A$, there is exactly one map $A \to 1$. The empty set $0$ is an \textbf{initial object}: for every set $A$, there is exactly one map $0 \to A$. Thinking in terms of arrows makes ``elements'' and ``emptiness'' uniform across categories.
