...
A \textbf{matching} is a set of edges with no shared endpoints. A \textbf{maximum matching} has the largest possible size. A \textbf{perfect matching} covers every vertex.
\end{definition}

\begin{theorem}[Hall's marriage theorem]
Let $G = (X \cup Y, E)$ be a bipartite graph. There is a matching that covers all vertices of $X$ iff for every subset $S \subseteq X$:
\[
|N(S)| \geq |S|
\]
...
\begin{example}
Let $X = \{1,2,3\}$, $Y = \{a,b,c\}$, and edges $\{1a, 1b, 2b, 3b, 3c\}$. For any $S \subseteq X$:
\[
...
\begin{remark}
Maximum matchings can be found by \textbf{augmenting paths}: repeatedly flip edges along a path that alternates between unmatched and matched edges to increase the matching size.
\end{remark}

\subsection*{Network flows}

\begin{definition}[Flow network]
A \textbf{flow network} is a directed graph with a source $s$, sink $t$, and nonnegative capacities $c(u,v)$ on edges.
...
\end{itemize}
The \textbf{value} of the flow is the total outflow from $s$.
\end{definition}

\begin{definition}[Cut]
An $s$-$t$ \textbf{cut} is a partition $(S, T)$ with $s \in S$ and $t \in T$.
Its \textbf{capacity} is $\sum_{u \in S, v \in T} c(u,v)$.
\end{definition}

\begin{theorem}[Max-flow min-cut]
The maximum value of an $s$-$t$ flow equals the minimum capacity of any $s$-$t$ cut.
\end{theorem}

\begin{remark}
The Ford--Fulkerson method repeatedly finds an augmenting path in the residual graph until no such path remains.
\end{remark}
...
\begin{enumerate}
  ...
  \item Let $X = \{1,2,3\}$ and $Y = \{a,b,c\}$ with edges $\{1a, 1b, 2b, 2c, 3c\}$. Use Hall's theorem to decide whether there is a matching that covers $X$, and find one if it exists.

  \item Find a maximum matching in the bipartite graph with $X=\{u,v,w\}$, $Y=\{x,y,z\}$, and edges $\{ux, uy, vx, vy, wz\}$.

  \item Compute the maximum flow value in a network with edges $s \to a$ (3), $s \to b$ (2), $a \to b$ (1), $a \to t$ (2), $b \to t$ (3). Give one maximum flow and a minimum cut.
\end{enumerate}
