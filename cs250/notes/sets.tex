...
Two quick consequences of the definition that we'll use later. First, the identity function $\mathrm{id}_A : A \to A$ is a bijection (it's its own inverse). Second, if $f : A \to B$ and $g : B \to C$ are bijections, then so is their composition $g \circ f : A \to C$, with inverse $f^{-1} \circ g^{-1}$. (Check this by composing: $(f^{-1} \circ g^{-1}) \circ (g \circ f) = f^{-1} \circ (g^{-1} \circ g) \circ f = f^{-1} \circ f = \mathrm{id}_A$.) So bijections compose, and this means ``has the same cardinality as'' is a transitive relation: if $|A| = |B|$ and $|B| = |C|$, then $|A| = |C|$, with the witnessing bijection obtained by composition.

\subsection*{A gallery of arrow diagrams}
...
\begin{center}
\begin{tikzpicture}[>={Stealth[length=2mm]}, node distance=0.9cm]
  % valid function
  \begin{scope}
    \node at (0.6, 2.2) {\textbf{Valid function}};
    \node[circle, draw, minimum size=5mm] (a1) at (0, 1.4) {};
    \node[circle, draw, minimum size=5mm] (a2) at (0, 0.6) {};
    \node[circle, draw, minimum size=5mm] (a3) at (0, -0.2) {};
    \node[circle, draw, minimum size=5mm] (b1) at (1.2, 1.6) {};
    \node[circle, draw, minimum size=5mm] (b2) at (1.2, 0.8) {};
    \node[circle, draw, minimum size=5mm] (b3) at (1.2, 0.0) {};
    \node[circle, draw, minimum size=5mm] (b4) at (1.2, -0.8) {};
    \draw[->] (a1) -- (b1);
    \draw[->] (a2) -- (b2);
    \draw[->] (a3) -- (b3);
    \node at (-0.4, -1.3) {$A$};
  ...
  \begin{scope}[xshift=4.5cm]
    \node at (0.6, 2.2) {\textbf{\emph{Not} a function}};
    \node[circle, draw, minimum size=5mm] (c1) at (0, 1.4) {};
    \node[circle, draw, minimum size=5mm] (c2) at (0, 0.6) {};
    \node[circle, draw, minimum size=5mm] (c3) at (0, -0.2) {};
    \node[circle, draw, minimum size=5mm] (d1) at (1.2, 1.6) {};
    \node[circle, draw, minimum size=5mm] (d2) at (1.2, 0.8) {};
    \node[circle, draw, minimum size=5mm] (d3) at (1.2, 0.0) {};
    \node[circle, draw, minimum size=5mm] (d4) at (1.2, -0.8) {};
    \draw[->] (c1) -- (d1);
    \draw[->, red] (c2) -- (d2);
    \draw[->, red] (c2) -- (d3);
    \node at (-0.4, -1.3) {$A$};
    \node at (1.6, -1.3) {$B$};
    \node[text=red] at (0.6, -2.0) {\small (one input, two outputs)};
  \end{scope}
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>={Stealth[length=2mm]}, node distance=0.9cm]
  % injection
  \begin{scope}
    \node at (0.6, 2.2) {\textbf{Injection}};
    \node[circle, draw, minimum size=5mm] (a1) at (0, 1.4) {};
    \node[circle, draw, minimum size=5mm] (a2) at (0, 0.6) {};
    \node[circle, draw, minimum size=5mm] (a3) at (0, -0.2) {};
    \node[circle, draw, minimum size=5mm] (b1) at (1.2, 1.6) {};
    ...
    \node at (-0.4, -1.3) {$A$};
    \node at (1.6, -1.3) {$B$};
    \node at (0.6, -2.0) {\small (distinct inputs $\mapsto$ distinct outputs)};
  \end{scope}
  % surjection
  \begin{scope}[xshift=5.2cm]
    \node at (0.6, 2.2) {\textbf{Surjection}};
    \node[circle, draw, minimum size=5mm] (c1) at (0, 1.6) {};
    \node[circle, draw, minimum size=5mm] (c2) at (0, 0.8) {};
    \node[circle, draw, minimum size=5mm] (c3) at (0, 0.0) {};
    \node[circle, draw, minimum size=5mm] (c4) at (0, -0.8) {};
    \node[circle, draw, minimum size=5mm] (d1) at (1.2, 1.2) {};
    \node[circle, draw, minimum size=5mm] (d2) at (1.2, 0.4) {};
    \node[circle, draw, minimum size=5mm] (d3) at (1.2, -0.4) {};
    \draw[->] (c1) -- (d1);
    \draw[->] (c2) -- (d1);
    \draw[->] (c3) -- (d2);
    \draw[->] (c4) -- (d3);
    \node at (-0.4, -1.3) {$A$};
    \node at (1.6, -1.3) {$B$};
    \node at (0.6, -2.0) {\small (every output is hit)};
  \end{scope}
  % bijection
  \begin{scope}[xshift=10.4cm]
    \node at (0.6, 2.2) {\textbf{Bijection}};
    \node[circle, draw, minimum size=5mm] (e1) at (0, 1.2) {};
    \node[circle, draw, minimum size=5mm] (e2) at (0, 0.4) {};
    \node[circle, draw, minimum size=5mm] (e3) at (0, -0.4) {};
    \node[circle, draw, minimum size=5mm] (f1) at (1.2, 1.2) {};
    \node[circle, draw, minimum size=5mm] (f2) at (1.2, 0.4) {};
    \node[circle, draw, minimum size=5mm] (f3) at (1.2, -0.4) {};
    \draw[->] (e1) -- (f1);
    \draw[->] (e2) -- (f2);
    \draw[->] (e3) -- (f3);
    \node at (-0.4, -1.3) {$A$};
    \node at (1.6, -1.3) {$B$};
    \node at (0.6, -2.0) {\small (perfect pairing)};
  \end{scope}
\end{tikzpicture}
\end{center}
...
Yes. Move every current guest from room $n$ to room $n+1$. Room 1 is now free for the new guest. The explicit bijection is $n \mapsto n+1$: the occupant of room $n$ moves to room $n+1$, and the function is injective (different rooms to different rooms) and surjective onto $\{2, 3, 4, \ldots\}$ (everyone still has a room). In fact, you can fit infinitely many new guests the same way: use the bijection $n \mapsto 2n$ to move each existing guest to an even-numbered room, freeing up all the odd-numbered rooms for the new arrivals (guest $k$ of the new group goes to room $2k - 1$). Again, both maps are bijections---no one loses a room, no room is assigned twice. It's not that the hotel ``got bigger''; it's that a different bijection between guests and rooms works just as well as the original.
...
\section{Diagonalization, cashed out: the halting problem}

You'll see this theorem developed properly in CS 251, once we have a formal model of computation (Turing machines, register machines---it doesn't really matter which). But the \emph{argument} is purely Cantor-style diagonalization, and we have everything we need for it right now. All we need is the intuitive programmer's notion of ``a program''---think Python.

\begin{theorem}[Turing, 1936]
There is no program \texttt{halts(P, x)} that takes the source code of a program \texttt{P} and an input \texttt{x}, always terminates, and returns \texttt{True} if \texttt{P} halts on input \texttt{x} and \texttt{False} if \texttt{P} loops forever on input \texttt{x}.
\end{theorem}

\begin{proof}
Suppose, for contradiction, that such a program \texttt{halts} exists. Since programs are just strings of source code, we can feed one program's source code to another as input---that's what passing \texttt{P} to \texttt{halts} means. In particular, we can ask \texttt{halts(P, P)}: ``does \texttt{P} halt when given its own source code as input?''

Using \texttt{halts} as a subroutine, we can build a new program \texttt{diag}:
...
In words: \texttt{diag(P)} runs \texttt{halts(P, P)} to find out what \texttt{P} does on itself, then does the \emph{opposite}. If \texttt{P} halts on \texttt{P}, then \texttt{diag(P)} loops forever. If \texttt{P} loops forever on \texttt{P}, then \texttt{diag(P)} halts.

Now ask the fatal question: what does \texttt{diag(diag)} do?

\begin{itemize}
  \item Suppose \texttt{diag(diag)} halts. Then \texttt{halts(diag, diag)} must have returned \texttt{False}---because that's the only branch of \texttt{diag} that halts. But \texttt{halts(diag, diag) = False} means, by definition of \texttt{halts}, that \texttt{diag} does \emph{not} halt on input \texttt{diag}. Contradiction.
  \item Suppose \texttt{diag(diag)} loops forever. Then \texttt{halts(diag, diag)} must have returned \texttt{True}---because that's the only branch of \texttt{diag} that loops. But \texttt{halts(diag, diag) = True} means \texttt{diag} \emph{does} halt on input \texttt{diag}. Contradiction.
\end{itemize}

Either way we get a contradiction, so our assumption was wrong: no such \texttt{halts} exists.
\end{proof}

I want to pause and point out how this mirrors Cantor's diagonal argument, because the correspondence is \emph{tight}.

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Cantor} & \textbf{Turing} \\
\midrule
List of reals $r_1, r_2, r_3, \ldots$ & List of programs $P_1, P_2, P_3, \ldots$ \\
$n$th digit of $r_n$ & Behavior of $P_n$ on input $P_n$ \\
Construct $d$ differing at every diagonal & Construct \texttt{diag} doing opposite at every diagonal \\
$d$ is a real not in the list & \texttt{diag}'s row can't exist in the halts-table \\
\bottomrule
\end{tabular}
\end{center}

In both proofs you assume a completed list (of reals; of halt-decisions), you construct a new thing that \emph{deliberately disagrees} with the list at the $n$th position for every $n$, and then you derive a contradiction because the new thing was supposed to be on the list. Cantor uses it to show there are more reals than naturals; Turing uses it to show there is no algorithm that decides halting. The same trick, pointed at different targets.

The moral: diagonalization isn't a one-off cleverness for real numbers. It's a reusable weapon. Anywhere you have a ``list of all the things that decide some question,'' you can often construct a question whose answer deliberately disagrees with every decider in the list---and so the list can't have been complete. G\"odel's incompleteness theorem, Tarski's undefinability of truth, Rice's theorem, and a long list of others are all variations on this theme. Smullyan's book in the Further Reading section traces the family tree.
