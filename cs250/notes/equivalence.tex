...
\begin{itemize}
  ...
  \item \textbf{Graph isomorphism:} ignore vertex labels, keep structure. The graph with vertices $\{1, 2, 3\}$ and edges $\{(1,2), (2,3)\}$ is ``the same'' as the graph with vertices $\{a, b, c\}$ and edges $\{(a,b), (b,c)\}$---they're both paths of length 2.
...
\textbf{Case-insensitive strings.} Let $A$ be the set of all strings over the English alphabet, and define $s_1 \sim s_2$ iff $s_1$ and $s_2$ differ only in capitalization. Then $[\text{``Hello''}] = \{\text{``Hello''}, \text{``hello''}, \text{``HELLO''}, \text{``hELLO''}, \ldots\}$---all $2^5 = 32$ capitalizations of a five-letter string. The quotient set $A / {\sim}$ is the set of case-insensitive strings. When Python's \texttt{str.lower()} method converts a string to lowercase before comparing, it's choosing a \emph{representative} from each equivalence class.
...
\textbf{Hash maps.} When you use a dictionary in Python or a \texttt{HashMap} in Java, you're relying on a notion of key equality. Two keys are ``the same'' if they produce the same hash \emph{and} compare equal under \texttt{\_\_eq\_\_} or \texttt{.equals()}. The hash map doesn't distinguish between two keys that are equal in this sense, even if they're different objects in memory. This is an equivalence relation on the set of all possible key objects, and the hash map is organized by equivalence classes.
