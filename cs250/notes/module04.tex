...
\textbf{Reading map.} This module covers \textbf{Epp §3.1--3.4} (predicates, quantified statements, and arguments with quantified statements) and \textbf{§5.2} (the principle of mathematical induction; we return to this in Module 8). \textbf{Prerequisites:} Modules 1--3 (sets and functions, propositional logic, proof systems). \textbf{Relevant intermezzi:} ``More on Natural Induction'' (immediately after this module, walks through commutativity and associativity of $+$ inductively) and ``Quantifiers as Games'' (gives a game-semantic intuition for nested $\forall/\exists$). \textbf{Practice from Epp:} §3.1 exercises on translating English into symbolic statements; §3.2 exercises on negating quantified statements; §3.3--3.4 exercises on validity in predicate logic; §5.2 exercises on simple induction.
\end{minipage}}
\end{center}
...
\]
You don't have to prove anything with it---just write down the rule.\footnote{If your rule has three premises, you're on the right track. If it has two, you forgot a case. If it has four, you've invented something interesting and we should talk.}
