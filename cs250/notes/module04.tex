...
\bigskip
\textbf{Exercise 4.} \label{ex:m4:4} Prove that for all $n : \text{Nat}$, $n + 1 = \text{succ}\; n$, where $1 = \text{succ}\; 0$. This one is a bit of a trick: figure out whether you actually need induction here, or whether you can do it just by unfolding the definition of \texttt{+}. (Hint: think about which equation of \texttt{+} fires when the second argument is $\text{succ}\; 0$.)

\bigskip
\textbf{Exercise 5.} \label{ex:m4:5} Take the natural induction rule above and look at it side by side with the BNF $\text{Nat} := 0 \mid \text{succ Nat}$. Identify exactly which part of the rule comes from which case of the BNF. Then, by analogy, write down what the induction rule would look like for the type
\[
