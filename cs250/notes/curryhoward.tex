...
\begin{tikzpicture}[node distance=3.2cm, >=Stealth]
  \node (prop) {\textsf{Proposition}};
  \node (type) [below left of=prop, node distance=3cm] {\textsf{Type}};
  \node (prog) [below right of=prop, node distance=3cm] {\textsf{Program}};
  \draw[<->, thick] (prop) -- node[above left] {\small Curry--Howard} (type);
  \draw[<->, thick] (type) -- node[below] {\small ``inhabitant of''} (prog);
  \draw[<->, thick] (prop) -- node[above right] {\small ``proof of''} (prog);
\end{tikzpicture}
\end{center}
