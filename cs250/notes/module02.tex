...
\begin{itemize}
  ...
  \item \texttt{raining} is a valid formula, a single variable named \texttt{raining}---variables can be any identifier, not just single letters
\end{itemize}
...
\begin{itemize}
  ...
  \item $p \vee q$ is \emph{or}\index{disjunction@$\vee$ (disjunction)} and is true when \textbf{at least one} of p, q is true; the case where both are true still counts as true (this is \emph{inclusive} or, unlike everyday English ``or'' which often means ``one but not both'')
...
The truth tables match! So in classical propositional logic we don't even \emph{need} implication as a primitive---it can be defined as $\lnot p \vee q$. Worth flagging that this equivalence is a feature of classical logic specifically: in constructive logic, where excluded middle isn't available, $p \to q$ and $\lnot p \vee q$ come apart. For this course we're staying classical, but if you ever read about constructive or intuitionistic logic, the fact that these two formulas are \emph{not} interchangeable there is one of the first surprises.
...
\begin{enumerate}
  ...
  \item \textbf{Misreading the scope of $\lnot$.} Without parentheses, $\lnot p \to q$ means $(\lnot p) \to q$---``if not-$p$ then $q$''---not $\lnot(p \to q)$. This catches students constantly when they translate English like ``it's not the case that $p$ implies $q$'' into the logic; the correct translation is $\lnot(p \to q)$, with the parentheses. When negation is meant to apply to a whole implication (or disjunction, conjunction, etc.), parenthesize explicitly.
\end{enumerate}
