...
This has a couple of implications. First, there's no sense of
\emph{ordering} in sets: $\{a,b,c\} = \{c,b,a\} = \{c,a,b\}$. Second,
it doesn't matter how you build the sets. You could get the ``set of
natural numbers divisible by four'' directly as
\[
  \{x \mid x \in \mathbb{N},\ x \mathbin{\%} 4 = 0\},
\]
or as a two-step process: first get
\[
  E = \{x \mid x \in \mathbb{N},\ x \mathbin{\%} 2 = 0\},
\]
and then filter again to get
\[
  \{x \mid x \in E,\ x \mathbin{\%} 4 = 0\}.
\]
Either way, you get the ``same'' set because they have the same elements.
