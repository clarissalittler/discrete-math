...
\begin{enumerate}
  ...
  \item Then $x = a \cdot vn + b \cdot um$ satisfies $x \equiv a \pmod m$ (because $vn \equiv 1 \pmod m$ by Bézout and $um \equiv 0 \pmod m$) and similarly $x \equiv b \pmod n$. \checkmark
\end{enumerate}
...
\medskip
\textbf{Looking ahead.} Module 7 introduces the two indirect proof techniques---contradiction and contrapositive---plus the Euclidean algorithm (which computes multiplicative inverses in $\mathbb{Z}/p\mathbb{Z}$ via Bézout, closing the loop with what we did here) and Hoare logic for program correctness. Before you get there, the equivalence-relation intermezzo uses modular congruence as its central example, so it's a good partner to this module.
\end{summarybox}
