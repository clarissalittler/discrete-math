...
\rmentry{Prerequisites.}{Modules 4 (natural induction), 8 (induction is recursion), and 9 (strong induction, which generalizes to recursive data where subparts are smaller but not adjacent).}
...
\begin{itemize}
  ...
  \item \textbf{Regular expressions and automata.} Every identity about regexes---``$(a|b)^* = a^* (b a^*)^*$,'' for example---is proved by structural induction on the regex grammar, or by induction on the automaton it compiles to. CS 251's formal languages material is essentially one long structural-induction exercise.
