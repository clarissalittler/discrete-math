...
\begin{readingmap}
\rmentry{Epp sections.}{This module doesn't correspond directly to an Epp section---Epp's §5.3 covers the closely related topic of recursive definitions on the naturals, but the more general ``structural induction on arbitrary recursive data'' is a computer-science-flavored topic that most discrete-math texts don't emphasize.}

\rmentry{Prerequisites.}{Modules 4 (natural induction) and 8 (induction is recursion). Module 9 (strong induction) is helpful background for two of the exercises below and for tree-on-tree arguments where the sub-parts aren't adjacent, but the main module text doesn't strictly require it.}

\rmentry{Relevant intermezzi.}{``The Algebra of Types'' (immediately after this module) explains why BNF definitions and induction principles line up perfectly---the pattern isn't a coincidence, it's a theorem about initial algebras.}

\rmentry{Practice from Epp.}{The exercises in §5.3 on recursive definitions are the closest match; beyond that, this module is the one where CS 250 most clearly points beyond the textbook.}
\end{readingmap}
...
\subsection*{Making ``induction is recursion'' concrete on this proof}
...
The statement we proved was ``for all lists $xs$, $P(xs)$'' where $P(xs)$ is ``for all $ys$, $\texttt{length}(xs \mathbin{++} ys) = \texttt{length}(xs) + \texttt{length}(ys)$.'' Imagine you wanted to \emph{compute} a proof of $P(xs)$ for a specific concrete list $xs$---say, a list of length three. What would that computation look like? It's a recursive function:
...
Reread the worked proof with this pseudocode next to it. The \texttt{if xs == Nil} branch is the base case of the induction. The recursive call \texttt{length\_append\_proof(xs\_tail)} \emph{is} the inductive hypothesis---``assume $P(xs')$,'' written in the vocabulary of ``recurse on the tail.'' The final equality chain in the \texttt{else} branch is the inductive step. No piece of the proof is missing from the code, and no piece of the code is missing from the proof. They are not two similar things; they are the same object written in two notations.
...
Before we prove anything, let's look at a concrete binary tree so we have something to count. The tree $\texttt{Node}(\texttt{Node}(\texttt{Leaf}, \texttt{Leaf}), \texttt{Node}(\texttt{Leaf}, \texttt{Node}(\texttt{Leaf}, \texttt{Leaf})))$ drawn out looks like this (blue circles are internal \texttt{Node}s and gray squares marked \texttt{L} are \texttt{Leaf}s):
...
\begin{sidebar}{structural induction in programming, for real}
...
\begin{itemize}
  \item \textbf{Compilers.} Every pass---parsing, type-checking, optimization, code generation---walks a recursive datatype (the abstract syntax tree, or the intermediate representation, or the control-flow graph). When the compiler team proves a transformation preserves meaning, they do structural induction on the AST. ``If optimization $O$ is correct on each sub-expression, it's correct on the whole expression'' is a structural induction step.
  \item \textbf{Type systems.} ``Progress and preservation'' is the standard soundness theorem for a typed programming language, and both halves are structural inductions on typing derivations (or on program syntax, or both). This is how we know a well-typed program ``can't go wrong.'' You'll see these proofs laid out in books like \emph{Types and Programming Languages} (Pierce) and \emph{Software Foundations} (Pierce et al.), where they're written down rigorously in Coq and are sometimes dozens of cases long.
  \item \textbf{Interpreters and evaluators.} Writing an interpreter is structural recursion on the abstract syntax; proving the interpreter correct is structural induction on the same syntax. The definitions of $\texttt{eval}$ and $P(e)$ have the same shape, one returning a value and the other returning a proof.
  \item \textbf{Static analyzers.} Tools like Facebook's Infer (separation logic), Clang Static Analyzer, SpotBugs, and ESLint all walk syntax trees checking an invariant. When the tool's authors prove the analysis is sound, they do structural induction on program syntax.
  \item \textbf{Regular expressions and automata.} Every identity about
  regexes is proved by structural induction on the regex grammar, or by
  induction on the automaton it compiles to. For example, identities like
  ``$(a|b)^* = a^* (b a^*)^*$'' are structural statements about regular
  expressions. CS 251's formal languages material is essentially one
  long structural-induction exercise.
  \item \textbf{DSLs and embedded languages.} Any time you write a small language inside another language---SQL queries, GraphQL schemas, a configuration grammar, a shader language---you're defining a BNF. Anything you want to prove or enforce about programs in that DSL is a structural induction on that grammar.
\end{itemize}
...
\subsection*{When the recursive case has variable structure: rose trees}

All the datatypes we've handled so far have recursive cases that carry a \emph{fixed} number of recursive sub-parts: \texttt{Cons} has one tail, \texttt{Node} has two subtrees, binary connectives have two subformulas. The recipe ``one IH per recursive occurrence'' is easy to apply when the ``number of occurrences'' is a small constant.

What about datatypes where a single constructor holds \emph{a whole list} of recursive sub-parts? The classic example is a \emph{rose tree}\index{rose tree}---a tree where each internal node has an arbitrary number of children, not just two:
\begin{lstlisting}
RoseTree A := Leaf(A) | Branch(List (RoseTree A))
\end{lstlisting}
A \texttt{Leaf} holds a value of type $A$; a \texttt{Branch} holds a list of rose-tree children (the list might be empty, or might have a hundred). Any filesystem directory structure, HTML DOM tree, or s-expression has roughly this shape.

The induction principle for rose trees needs to handle the \texttt{Branch} case carefully: you don't get ``an IH per child'' because you don't know how many children there are. Instead, you get an IH that says ``the property holds for \emph{every} element of the \texttt{List}.'' Concretely:
...
Let's see this with a small example. Define the \emph{size} of a rose tree to be the total number of \texttt{Leaf}s:
\begin{lstlisting}
size(Leaf(a))        = 1
size(Branch(cs))     = sum of size(c) over each c in cs
                       (which equals 0 if cs is Nil)
\end{lstlisting}
And define the \emph{count of \texttt{Branch} nodes}:
\begin{lstlisting}
branches(Leaf(a))    = 0
branches(Branch(cs)) = 1 + sum of branches(c) over each c in cs
\end{lstlisting}

\textbf{Claim.} For every rose tree $t$, $\texttt{size}(t) \geq 1$.

\textbf{Proof} by structural induction on $t$.

\emph{Base case} ($t = \texttt{Leaf}(a)$): $\texttt{size}(\texttt{Leaf}(a)) = 1 \geq 1$. \checkmark

\emph{Inductive step} ($t = \texttt{Branch}(cs)$): The inductive hypothesis says $\texttt{size}(c) \geq 1$ for every $c$ in the list $cs$.

We split on whether $cs$ is empty.
\begin{itemize}
  \item If $cs = \texttt{Nil}$, then $\texttt{size}(\texttt{Branch}(\texttt{Nil})) = 0$ by the definition (empty sum). But wait---$0 \not\geq 1$. Our claim is false for empty-branch rose trees! The definition of \texttt{RoseTree} allows \texttt{Branch(Nil)}---a ``branch with no children''---and such a tree has size $0$.
\end{itemize}

The proof \emph{should} fail here, and the lesson is genuine: rose trees admit \texttt{Branch}es with no children, and those have size $0$. The right claim for \texttt{RoseTree} as defined is something like ``every rose tree has at least one \texttt{Leaf} \emph{or} is an empty \texttt{Branch}.'' Alternatively, if we want the $\texttt{size} \geq 1$ claim to hold, we have to rule out empty branches at the BNF level: e.g., require that \texttt{Branch} carry a \emph{non-empty} list of children. Either fix is available; they make different mathematical objects.

This is a feature, not a bug, of structural induction: the structure of the BNF forces you to confront edge cases that might otherwise hide. Students who say ``obviously a tree has at least one leaf'' have usually forgotten the \texttt{Branch(Nil)} case.

Let's instead prove something that actually is true of the datatype as given.

\textbf{Claim.} For every rose tree $t$, $\texttt{size}(t) = \texttt{branches}(t) + \ell(t)$, where $\ell(t)$ is the number of \texttt{Leaf} nodes in $t$ (an easy separate recursive definition---try writing it).
...
\begin{enumerate}
  ...
  \item \textbf{Generalizing the wrong variable.} When proving ``for all \texttt{xs} and \texttt{ys}, $\ldots$'', you usually induct on one of them with the \emph{other} universally quantified inside the predicate $P(\texttt{xs})$. If you fix a particular \texttt{ys} and then try to induct on \texttt{xs}, you might find your proof goes through---but you've only proved the result for that particular \texttt{ys}. Concretely: the \emph{right} predicate for induction on \texttt{xs} in the length-append proof is
\[
  P(\texttt{xs}) := \forall \texttt{ys}.\; \texttt{length}(\texttt{xs} \mathbin{++} \texttt{ys}) = \texttt{length}(\texttt{xs}) + \texttt{length}(\texttt{ys}).
\]
The \emph{wrong} predicate is the same statement with some specific \texttt{ys} (say, \texttt{Nil}) silently fixed. The second version can be induced on and the algebra goes through, but the thing you've proved is just ``the equation holds when \texttt{ys} = \texttt{Nil}''---not the universal claim. The quantifier has to live \emph{inside} $P$, otherwise the inductive hypothesis doesn't give you anything about other values of \texttt{ys}.
...
\textbf{Exercise 1.}\label{ex:m10:1} Define a recursive function
$\texttt{reverse}$ on lists, following the two cases of the BNF. Hint:
you will need \texttt{append} (\texttt{++}) in the recursive case. Then
compute
\[
  \texttt{reverse}(\texttt{Cons}(1,
    \texttt{Cons}(2, \texttt{Cons}(3, \texttt{Nil}))))
\]
step by step, showing each unfolding of the definition.
...
\bigskip
\textbf{Exercise 7b.}\label{ex:m10:7b} (Rose trees.) Using the $\texttt{RoseTree}$ datatype from the rose-tree subsection, prove by structural induction: for every rose tree $t$, the total number of constructors appearing in $t$ (counting each \texttt{Leaf} and each \texttt{Branch} once) equals $\texttt{branches}(t) + \texttt{size}(t)$. Start by writing a recursive definition of the ``total constructor count'' function, then do the induction. The \texttt{Leaf} case is immediate. The \texttt{Branch}$(cs)$ case is where the variable-arity IH actually gets exercised: you need to apply the IH to every child in the list $cs$ at once, not to a single ``previous'' child. State the IH precisely before using it---``for every child $c$ occurring in $cs$, the property holds''---and keep that quantifier visible in your proof.
...
\bigskip
\textbf{Exercise 9.}\label{ex:m10:9} (A real semantic property.) Say a propositional formula is \emph{negation-free} if it is built from $T$, $F$, variables, and the connectives $\wedge$, $\vee$ only (i.e., the BNF excludes $\lnot$ and $\to$). Say a boolean function $f$ is \emph{monotone in variable $x$} if flipping $x$ from $F$ to $T$ (with all other variables held fixed) never flips the output of $f$ from $T$ to $F$. Prove by structural induction on the negation-free BNF:
\[
...
\bigskip
\textbf{Exercise 10.}\label{ex:m10:10} A binary tree is \emph{perfect} if
every non-leaf node has two subtrees of the same height. Define
\[
  \texttt{height}(\texttt{Leaf}) = 0,\qquad
  \texttt{height}(\texttt{Node}(l, r))
    = 1 + \max(\texttt{height}(l), \texttt{height}(r)).
\]
Prove by structural induction that every perfect binary tree $t$
satisfies $\texttt{leaves}(t) = 2^{\texttt{height}(t)}$. Hint: the
definition of ``perfect'' is itself inductive: \texttt{Leaf} is perfect,
and $\texttt{Node}(l, r)$ is perfect if $l$ and $r$ are both perfect and
have the same height. Induct on the structure of the perfect tree.
