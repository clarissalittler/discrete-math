...
\bigskip
\textbf{Exercise 3.} \label{ex:m10:3} Define a function $\texttt{vars}(f)$ that returns the number of variable occurrences in a propositional formula (following the BNF from Module~2). For instance, $\texttt{vars}(x \wedge y) = 2$ and $\texttt{vars}(x \wedge x) = 2$ (we're counting occurrences, not distinct variables). Then state---but you don't need to prove---what the structural induction principle would require you to show in order to prove some property $P(f)$ for all formulas $f$. How many base cases? How many inductive steps? How many IHs in each?
