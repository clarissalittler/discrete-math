...
\begin{tikzpicture}[
  ...
  \node[mod, below=of m1] (m2) {Module 2\\Prop.\ Logic};
  ...
  \node[mod, below=of m7] (m8) {Module 8\\Induction};
...
\noindent\textbf{How to read the diagram.} Solid arrows along the middle column tell you the module reading order---Module 1 comes first, each subsequent module depends on the ones above it. Dashed arrows to the right column tell you which intermezzi are best read alongside each module. ``Alongside'' means: read the module first, then the intermezzo, before moving on to the next module. The intermezzi genuinely enrich the spine, and several of them call back to earlier material in ways that reinforce understanding.
...
\midrule
§1.1--1.3                          & Module 1 --- Sets, Functions, Proof-Writing \\
...
\begin{itemize}
  \item $\lnot$ and $\sim$ both mean negation. These notes use $\lnot$ in displayed math and $\sim$ in \texttt{lstlisting} code blocks. Epp uses $\sim$. They mean the same thing.
...
\begin{itemize}
  ...
  \item \textbf{Write your proofs in prose.} A proof is a short essay, not a column of symbols. If you can't explain each step in a sentence, you probably don't understand the step. The Module 1 ``how to write a proof'' section is worth re-reading after every few proofs you write.
